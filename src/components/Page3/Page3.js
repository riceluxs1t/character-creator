...
const Page3 = (props) => {
  return (
    <Page>
      ...
      <Paragraph>
        그러나 내 멋대로 살 수 있는 내 삶에서 나는 대체로 갈 데까지 가려는 쪽에
        가까워. 나는 대학원 문턱에도 가본 적이 없지만 아마도 PhD를 따려면 자신의
        연구 주제에 대해서 갈 데까지 가봐야 하지 않을까. 그건 (아마 너도 공감할
        텐데) 힘든 일이지. 그러려면 자신의 삶을 어느 정도는 맷돌처럼 한 가지
        영역에 갈아 넣어야 하니까.
      </Paragraph>
      <Paragraph>
      ...
      <Paragraph>
        ...
        대한 죄책감도 전혀 가지지 않고, 츄루르릅 소리를 내면서도 남 신경 쓰지
        않을 거야. 아이스크림은 건강에 치명적인 만큼 그 정도로 맛있으니까.
        무진장 기간 동안 우리가 맷돌처럼 진지하게 우리 시간을 갈아서 콩국수같이
        건강한 음식도 만들어 먹겠지만, 나는 우리가 아이스크림을 빨듯이 별생각
        없이 그 자체로 달콤한 시간도 더 많이 누렸으면 좋겠다. 이 편지를
        주고받듯이.
      </Paragraph>
